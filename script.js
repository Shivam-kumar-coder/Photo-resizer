// ================= QUICKPIC - COMPRESS WITH PROGRESS SPINNER =================
...
function showSuccess(msg) {
  ...
  Object.assign(t.style, {position:'fixed',top:'18px',right:'18px',background:'#16a34a',color:'#fff',padding:'10px 16px',borderRadius:'8px',zIndex:99999});
  document.body.appendChild(t); setTimeout(() => t.remove(), 3000);
}
function showError(msg) {
  const t = document.createElement('div'); t.className = 'toast error'; t.textContent = msg;
  Object.assign(t.style, {position:'fixed',top:'18px',right:'18px',background:'#dc2626',color:'#fff',padding:'10px 16px',borderRadius:'8px',zIndex:99999});
...
resetBtn?.addEventListener('click', () => { files=[]; fileInput.value=''; thumbGrid.innerHTML=''; thumbsArea.classList.add('hidden'); document.getElementById('select-area').style.display='block'; });
...
function createSpinner(msg){
  ...
  Object.assign(s.style,{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',padding:'14px 20px',background:'#fff',border:'2px solid #16a34a',borderRadius:'12px',boxShadow:'0 6px 18px rgba(0,0,0,0.12)',textAlign:'center',zIndex:99999,fontWeight:600});
  s.innerHTML = `<div class="spinner" style="border:4px solid #f3f3f3;border-top:4px solid #16a34a;border-radius:50%;width:32px;height:32px;margin:0 auto 8px auto;animation:spin 1s linear infinite;"></div>${msg}`;
...
// --- Compress KB/MB (Tera Genius Method) ---
async function compressToTargetSize(file, targetKB){
  ...
  for(let attempt=0; attempt<25; attempt++){
    ...
    else { size+=Math.round(step/2); quality=Math.min(0.95,quality+0.02); }

    if(targetKB>1024) quality=Math.max(0.5,quality-0.03);
  }
...
document.getElementById('runKb')?.addEventListener('click',async()=>{
  ...
  createSpinner(`Compressing ~${targetKB} KB ...`);
  ...
  removeSpinner();
  success?showSuccess(`Success: ${success} image(s) → ~${targetKB} KB`):showError("Compression failed");
});